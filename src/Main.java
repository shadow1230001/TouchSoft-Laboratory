...
public class Main {
        ...
        public static void main(String[] args) {
            ...
            List<Integer> res = normalizeEx(splitEx(calculateOutputParams(inputParams, outputParams)));
            show("res", res);
            int vres = 0;
        ...
        private static void checkPrev(List<Integer> inputParams, List<Integer> outputParams, int index) {
            ...
            if (prev > current) {
                ...
                outputParams.set(index, outputParams.get(index-1));
            }
        }
        ...
        private static void normalizeMinMax(int f, List<List<Integer>> lists) {
        ...
        private static void normalizeMaxMin(int f, List<List<Integer>> lists) {
            for(int i = f; i < lists.size(); i+=2) {
                int leftdif = 0;
                int rightdif = 0;
                List<Integer> list = lists.get(i);
        ...
        private static List<List<Integer>> splitEx(List<Integer> outputParams) {
            List<List<Integer>> res = new ArrayList<List<Integer>>();
            int start = 0;
            for(int i = 0; i < outputParams.size() - 1; i++) {
                if (outputParams.get(i) == outputParams.get(i+1)){
                    res.add(outputParams.subList(start, i+1));
                    start = i + 1;
                }
            }
            res.add(outputParams.subList(start, outputParams.size()));
        ...
        private static List<Integer> normalizeEx(List<List<Integer>> lists) {
            List<Integer> res = new ArrayList<Integer>();

            for(int i = 0; i < lists.size(); i++) {
                int min = findMin(lists.get(i));
                increment(lists.get(i), -min+1);
            }
